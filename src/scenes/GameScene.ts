...
export class GameScene extends PIXI.Container {
    ...
    private createUserDealerTextCount() {
        ...
        this.dealerCountText.y =
            this.tableHeight / 2 - this.tableHeight / 2 / 2;
    ...
    private playDealerHitSequence() {
        ...
        gsap.to(card, {
            x:
                this.cardXCoordinate +
                (this.dealerCards.length - 1) * this.cardSpacing,
    ...
    private verifyDealerGameOver() {
        let playerCardCount = this.checkHandValue(this.userCards);
        let dealerCardCount = this.checkHandValue(this.dealerCards);
    ...
    public dealCardToPlayer() {
        ...
        gsap.to(card, {
            x:
                this.cardXCoordinate +
                (this.userCards.length - 1) * this.cardSpacing,
    ...
    public shareCards() {
        ...
        gsap.delayedCall(
            (this.dealerCards.length + 1) * 0.4,
            this.revealCard.bind(this)
        );
    }

    private updateUserCount(count: number = 0) {
        this.userCount += count;

        this.userCountText.text = `${this.userCount} - USER`;
    }

    private updateDealerCount(count: number = 0) {
    ...
    private checkHandValue(cards: Card[]) {
        let aceCount = 0;
